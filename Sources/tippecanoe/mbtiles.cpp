...
sqlite3 *mbtiles_open(char *dbname, int forcetable) {
	sqlite3 *outdb;

	if (sqlite3_open(dbname, &outdb) != SQLITE_OK) {
		fprintf(stderr, "tippecanoe: %s: %s\n", dbname, sqlite3_errmsg(outdb));
		exit(EXIT_FAILURE);
	}

	char *err = NULL;
	if (sqlite3_exec(outdb, "PRAGMA synchronous=0", NULL, NULL, &err) != SQLITE_OK) {
		fprintf(stderr, "tippecanoe: async: %s\n", err);
		exit(EXIT_FAILURE);
	}
	...
	if (sqlite3_exec(outdb, "CREATE TABLE metadata (name text, value text);", NULL, NULL, &err) != SQLITE_OK) {
		fprintf(stderr, "tippecanoe: create metadata table: %s\n", err);
		if (!forcetable) {
			exit(EXIT_FAILURE);
		}
	}
...
void mbtiles_close(sqlite3 *outdb) {
	char *err;

	if (sqlite3_exec(outdb, "ANALYZE;", NULL, NULL, &err) != SQLITE_OK) {
		fprintf(stderr, "tippecanoe: ANALYZE failed: %s\n", err);
		exit(EXIT_FAILURE);
	}
	if (sqlite3_close(outdb) != SQLITE_OK) {
		fprintf(stderr, "tippecanoe: could not close database: %s\n", sqlite3_errmsg(outdb));
		exit(EXIT_FAILURE);
	}
}
