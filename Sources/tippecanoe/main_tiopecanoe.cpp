...
#define PARSE_MAX (1LL * 1024 * 1024 * 1024)
	...
	int written = traverse_zooms(fd, size, meta, stringpool, &midx, &midy, maxzoom, minzoom, basezoom, outdb, droprate, buffer, fname, tmpdir, gamma, full_detail, low_detail, min_detail, meta_off, pool_off, initial_x, initial_y, simplification, layermaps, persent);
...
int runMain(int argc, char **argv) {
	...
	files_open_at_start = open(tmpdir, O_RDONLY);
	...
	sqlite3 *outdb = mbtiles_open(outdir, forcetable);
	int ret = EXIT_SUCCESS;
	...
	ret = read_input(sources, name ? name : outdir, layer, maxzoom, minzoom, basezoom, basezoom_marker_width, outdb, &exclude, &include, exclude_all, droprate, buffer, tmpdir, gamma, read_parallel, forcetable, attribution, gamma != 0, 0);
...
int render_vector_tiles(RenderOptions options, double *persent) {
    ...
    init_cpus(options.tmpdir);
    
    
    files_open_at_start = open(options.tmpdir, O_RDONLY);
    close(files_open_at_start);
    ...
    if (full_detail < min_detail || low_detail < min_detail) {
        fprintf(stderr, "tippecanoe: Full detail and low detail must be at least minimum detail\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (options.output == NULL) {
        fprintf(stderr, "tippecanoe: must specify -o out.mbtiles\n");
        exit(EXIT_FAILURE);
    }
    ...
    sqlite3 *outdb = mbtiles_open(options.output, forcetable);
    int ret = EXIT_SUCCESS;
    
    if (options.input) {
        struct source src;
        src.layer = "";
        src.file = std::string(options.input);
        sources.push_back(src);
    }
    
    ret = read_input(sources, name ? name : options.output, layer, maxzoom, minzoom, basezoom, basezoom_marker_width, outdb, &exclude, &include, exclude_all, droprate, buffer, options.tmpdir, gamma, read_parallel, forcetable, attribution, gamma != 0, persent);
    ...
    i = open(options.tmpdir, O_RDONLY);
    // i < files_open_at_start is not an error, because reading from a pipe closes stdin
    if (i > files_open_at_start) {
        fprintf(stderr, "Internal error: did not close all files: %d\n", i);
        exit(EXIT_FAILURE);
